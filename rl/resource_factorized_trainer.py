"""Training loop for Resource-Factorized DQN Agent."""

import numpy as np
import logging
from typing import List, Dict, Any, Optional

from .resource_factorized_agent import ResourceFactorizedDQNAgent, ResourceFactorizedAgentConfig
from .decomposed_env import DecomposedRewardEnv
...
class ResourceFactorizedDQNTrainer:
    """Trains the Resource-Factorized DQN Agent with independent CPU and Memory TD learning."""

    def __init__(
        self,
        agent_config: Optional[ResourceFactorizedAgentConfig] = None,
        env_config: Optional[EnvConfig] = None,
    ):
        self.agent = ResourceFactorizedDQNAgent(agent_config)
        self.env = DecomposedRewardEnv(env_config or EnvConfig())
        self.eval_scenarios = get_synthetic_evaluation_scenarios()

        self.best_eval_reward = float("-inf")
        self.checkpoint_path = "checkpoints/best_resource_factorized_dqn.pt"
    ...
    def generate_balanced_trace(
        ...
        """Generate a deterministic, balanced synthetic trace for training across 4 regimes."""
    ...
    def train(
        ...
        """Run the Resource-Factorized DQN training loop."""
        history = {
            "episode_rewards": [],
            "losses": [],
            "eval_rewards": [],
        ...
        for ep in range(1, num_episodes + 1):
            ...
            while not done:
                cpu_idx, mem_idx = self.agent.choose_branch_indices(state)
                joint_action = cpu_idx * self.agent.num_mem_actions + mem_idx

                next_state, reward, terminated, truncated, info = self.env.step(joint_action)
                done = terminated or truncated

                r_cpu = info.get("r_cpu", 0.0)
                r_mem = info.get("r_mem", 0.0)

                # Store separate r_cpu and r_mem in factorized replay buffer
                self.agent.memory.push(state, cpu_idx, mem_idx, r_cpu, r_mem, next_state, done)
                state = next_state
                ep_reward += reward
            ...
            if ep % eval_freq == 0 or ep == num_episodes:
                ...
                mean_eval_reward = float(np.mean([res["mean_reward"] for res in eval_results.values()]))
                ...
                mean_cpu = float(np.mean(mean_cpus)) if mean_cpus else 0.0
                mean_mem = float(np.mean(mean_mems)) if mean_mems else 0.0
                ...
                if mean_eval_reward > self.best_eval_reward:
                    ...
                    logger.info(
                        f"Saved new best resource-factorized model with Eval Rew: {mean_eval_reward:.2f}"
                    )
