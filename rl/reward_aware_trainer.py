"""Trainer for the Monolithic 16-Action DQN with Resource-Aware Decomposed Reward."""
...
class RewardAwareDQNTrainer:
    """Trains the monolithic 16-action DQN using the resource-aware decomposed reward signal."""

    def __init__(self, agent_config: DQNAgentConfig, env_config: EnvConfig):
        self.agent = DQNAgent(agent_config)
        self.env = DecomposedRewardEnv(env_config)
        self.eval_scenarios = get_synthetic_evaluation_scenarios()

        self.best_eval_reward = float('-inf')
        self.checkpoint_path = "checkpoints/best_reward_aware_dqn.pt"
    ...
    def train(self, num_episodes: int = 100, max_steps_per_episode: int = 50, eval_freq: int = 10) -> Dict[str, Any]:
        """Run the DQN training loop using decomposed reward feedback."""
        history = {
            "episode_rewards": [],
            "losses": [],
            "eval_rewards": [],
        ...
        for ep in range(1, num_episodes + 1):
            ...
            while not done:
                ...
                # env.step returns r_exp which is the exact sum of r_cpu and r_mem
            ...
            if ep % eval_freq == 0 or ep == num_episodes:
                ...
                if mean_eval_reward > self.best_eval_reward:
                    ...
                    logger.info(f"Saved new best reward-aware model with Eval Rew: {mean_eval_reward:.2f}")
